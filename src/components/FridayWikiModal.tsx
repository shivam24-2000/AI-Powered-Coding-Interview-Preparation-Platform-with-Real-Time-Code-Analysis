  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        position: 'relative',
        overflow: 'hidden',
        boxShadow: '0 40px 100px rgba(100, 80, 160, 0.2), inset 0 0 40px rgba(150, 118, 200, 0.05)',
        ...
        <div style={{ padding: '50px 50px 30px', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
            <div style={{ padding: '12px', background: 'rgba(150, 118, 200, 0.1)', border: '1px solid rgba(150, 118, 200, 0.2)', borderRadius: '16px' }}>
              <Brain size={32} className="glow-icon" color="#a78bce" />
            </div>
            <div>
              ...
              <p style={{ color: 'rgba(150, 118, 200, 0.8)', fontSize: '0.9rem', margin: '4px 0 0', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Behavioral Intelligence Core v4.2</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '0 50px 50px', overflowY: 'auto', flex: 1, position: 'relative', zIndex: 1 }} className="custom-scrollbar">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
              ...
              <div style={{ padding: '24px', background: 'linear-gradient(135deg, rgba(100, 80, 160, 0.1), rgba(6, 182, 212, 0.1))', border: '1px solid rgba(150, 118, 200, 0.2)', borderRadius: '24px' }}>
                <h4 style={{ margin: '0 0 12px 0', fontSize: '1rem', color: 'var(--landing-text-primary)', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Sparkles size={18} color="#a78bce" /> Easter Eggs
                </h4>
        ...
        <style>{`
          .glow-icon {
            filter: drop-shadow(0 0 8px rgba(150, 118, 200, 0.6));
            animation: pulse-glow 3s infinite ease-in-out;
          }
          @keyframes pulse-glow {
            0%, 100% { filter: drop-shadow(0 0 8px rgba(150, 118, 200, 0.4)); }
            50% { filter: drop-shadow(0 0 15px rgba(150, 118, 200, 0.8)); }
          }
          .friday-wiki-container .modal-close-btn:hover {
            background: rgba(150, 118, 200, 0.2) !important;
            color: #fff !important;
          }
...
const FeatureRow: React.FC<{ icon: React.ReactNode, title: string, desc: string }> = ({ icon, title, desc }) => (
  <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
    <div style={{ color: '#a78bce', marginTop: '4px' }}>{icon}</div>
    <div>
