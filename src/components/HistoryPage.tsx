    ...
    <div className="history-page-root" style={{ padding: '24px', paddingBottom: '80px', maxWidth: '1200px', margin: '0 auto', color: '#fff', minHeight: '100%', position: 'relative' }}>
      ...
      <style>{`
        .heatmap-square:hover {
          transform: scale(1.3);
          z-index: 10;
          box-shadow: 0 0 10px rgba(150, 118, 200, 0.5);
        }
      `}</style>
              ...
              <div className="history-heatmap-legend-text" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)' }}>
                ...
                {[0, 2, 5, 8].map(level => {
                  const color = level === 0 ? 'rgba(255,255,255,0.05)' :
                    level < 3 ? 'rgba(150, 118, 200, 0.3)' :
                      level < 6 ? 'rgba(150, 118, 200, 0.6)' : 'rgba(150, 118, 200, 1)';
                      ...
                      <div key={weekIdx} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {Array.from({ length: 7 }).map((_, dayIdx) => {
                          ...
                          const color = data?.count === 0 ? 'rgba(255,255,255,0.05)' :
                            data?.count < 3 ? 'rgba(150, 118, 200, 0.3)' :
                              data?.count < 6 ? 'rgba(150, 118, 200, 0.6)' : 'rgba(150, 118, 200, 1)';
                          return (
                            <div
                              key={dayIdx}
          ...
          <div className="glass-panel history-section-card" style={{ padding: '24px', borderRadius: '16px', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.04)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {topTags.map(([tag, count]) => (
                <div key={tag} style={{ padding: '8px 16px', borderRadius: '12px', background: 'rgba(150, 118, 200, 0.08)', border: '1px solid rgba(150, 118, 200, 0.15)', display: 'flex', alignItems: 'center', gap: '8px' }}>
